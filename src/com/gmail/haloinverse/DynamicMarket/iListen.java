			...
			private boolean showHelp(CommandSender sender, String topic)
			{
				...
				if ((Misc.isEither(topic.split(" ")[0],"tags","tag")) && ((hasPermission(sender, "items.add") || hasPermission(sender, "items.update"))))
				{
					...
					else
					{
						...
						return true;
						/*
						message.send("{CMD} n:{}|{CMD}name:{} - Name/rename item");
						message.send("{CMD} bp:{} - Base purchase price @ (stock=0)");
						message.send("{CMD} s:{} - Current stock level. (Can be negative)");
						message.send("{CMD} cb:{} - Can be bought? (=y|n)");
						message.send("{} Also: {CMD}buyOK{}, {CMD}!nobuy{}, {CMD}nobuy{}, {CMD}!buyOK");
						message.send("{CMD} cs:{} - Can be sold? (=y|n)");
						message.send("{} Also: {CMD}sellOK{}, {CMD}!nosell{}, {CMD}nosell{}, {CMD}!sellOK");
						message.send("{CMD} fixed{} - Sets {CMD}s{}/{CMD}sl{}/{CMD}sh{}/{CMD}sf{}/{CMD}sc{} so price remains fixed.");
						message.send("{CMD} flat{} - Sets {CMD}s{}/{CMD}sl{}/{CMD}sh{}/{CMD}sf{}/{CMD}sc{} so price remains flat.");
						message.send("{CMD} float{} - Sets {CMD}sf{}/{CMD}sc{}/{CMD}pf{}/{CMD}pc{} so price can fluctuate with stock.");
						message.send("{CMD} renorm{} - Sets stock to 0, changes BasePrice to preserve current buy/sell price.");
						message.send("{CMD} v:{} - Volatility");
						message.send("{CMD} iv:{} - Inverse volatility (converted)");
						message.send("{CMD} st:{} - Sales tax on sold item");
						message.send("{CMD} sl:{} - Lowest stock before purchases fail.");
						message.send("{CMD} sh:{} - Highest stock before sales fail.");
						message.send("{CMD} sf:{} - Lowest stock level possible");
						message.send("{CMD} sc:{} - Highest stock level possible");
						message.send("{CMD} pf:{} - Maximum base price");
						message.send("{CMD} pc:{} - Minimum base price");
						*/
					}
				}
...
/* 53 */     return iConomy.db.get_balance(name);
/*    */   }
/*    */ 
/*    */   private void set_balance(String name, int amount) {
/* 57 */     iConomy.db.set_balance(name, amount);
/*    */   }
/*    */ 
/*    */   private void show_balance(Player player) {
/* 61 */     plugin.iC.l.showBalance(player.getName(), player, true);
/*    */   }
/*    */ 
 ...
 * Qualified Name:     com.nijikokun.bukkit.SimpleShop.iListen
